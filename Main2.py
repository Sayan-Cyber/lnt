...
# def cover_win ():                                                 #________________________________function to call cover sheet________________________________________________
#     cover=Toplevel()
#     cover.geometry('320x200+800+300')
#     cover.title('Cover Sheet')
#     cover.config(bg=w)
#     cover.resizable(False,False)
#     frame= Frame(cover,bg=w)
#     frame.place(relx=.5,rely=.48,anchor=CENTER)
                    
    
#     ddate= Entry(frame,width=20, relief=SUNKEN,borderwidth=.5,highlightcolor='#00FFC6',highlightthickness=1)
#     ddate.grid(row=1,column=1,pady=4)
#     cdate= Entry(frame,width=20, relief=SUNKEN, borderwidth=.5,highlightcolor='#00FFC6',highlightthickness=1)
#     cdate.grid(row=2,column=1,pady=4)
#     adate= Entry(frame,width=20, relief=SUNKEN,borderwidth=.5,highlightcolor='#00FFC6',highlightthickness=1)
#     adate.grid(row=3,column=1,pady=4)
  

  
    
    
#     Label(cover,text='Cover Sheet',font=('Readex Pro Medium',12),width=340,height=1, bg='#13c2f2' ,fg=w).pack(anchor=N)
    

    
    
#     ddateicon= Image.open('Additional_File\icons\ddate.png')
#     coverphoto1 = ImageTk.PhotoImage(ddateicon)
#     l1=Label(frame,text="Designed Date : ", font=('Readex Pro Medium',13), fg='#0E185F' ,bg=w,image=coverphoto1,compound=LEFT,padx=4)
#     l1.grid(row=1,column=0,sticky=W)
#     l1.image=coverphoto1
#     cdateicon= Image.open('Additional_File\icons\cdate.png')
#     coverphoto2 = ImageTk.PhotoImage(cdateicon)
#     adateicon= Image.open('Additional_File\\icons\\adate.png')
#     coverphoto3 = ImageTk.PhotoImage(adateicon)
    
#     l2=Label(frame,text="Checked Date : ", font=('Readex Pro Medium',13), fg='#0E185F' ,bg=w,image=coverphoto2,compound=LEFT)
#     l2.image=coverphoto2
#     l2.grid(row=2,column=0,padx=4,sticky=W)
#     l3=Label(frame,text="Approved Date : ", font=('Readex Pro Medium',13), fg='#0E185F' ,bg=w,image=coverphoto3,compound=LEFT)
#     l3.image= coverphoto3
#     l3.grid(row=3,column=0,padx=4,sticky=W),
    
    
    
#     def sav_e():                                                                    #function of save button in cover sheet________________________________________________________________
#         if os.path.isfile('Additional_file\\temp\\temp.docx')== TRUE:
#             c_d_ate= cdate.get()
#             a_d_ate= adate.get()
#             d_d_ate= ddate.get()
#             doc= DocxTemplate('Additional_file\\temp\\temp.docx')
#         else:
#             messagebox.showwarning('File not found',"Main Sheet was not filled.\nPlease fill the main sheet first!",parent=cover)    
#             return;
#         if len(c_d_ate)!=8 or len(a_d_ate)!=8 or len(d_d_ate)!=8:
#             messagebox.showwarning('Wrong Input','Please Enter Date in "DD.MM.YY" format',parent=cover)
#             return;
#         else:   
#             context={'c_date':c_d_ate,'d_date': d_d_ate, 'a_date':a_d_ate }
#             doc.render(context)
#         files = [('Word Document','*.docx')]
#         file = asksaveasfilename(filetypes = files, defaultextension=files,parent=cover)
#         if file=='':
#                 messagebox.showinfo('Error', 'Cover Sheet was not Saved :(',parent=cover)
#         else:
#             doc.save(file)
#             cover.destroy()
#             os.remove('Additional_file\\temp\\temp.docx')
#     Button(cover,text="Save",fg=b,font=('Josefin Sans',10),cursor='hand2', bg=w,borderwidth=0,command=sav_e, activebackground='#f2f2f2',image= photoimage2,compound=RIGHT).place(relx=.5,rely=.9, anchor=CENTER)


# def maintenance():                                                                  #temporary window for under development windows or Page______________________________________________________________________
#     mntnce= Toplevel()
#     mntnce.state('zoomed')
#     mntnce.geometry('1450x800+300+100')
#     mntnce.configure(background=w)
#     def quit():
#         mntnce.destroy()
#     mntnce.bind('<Escape>',quit)
#     imagesad = Image.open("Additional_File\\sad.png")
#     testsad= ImageTk.PhotoImage(imagesad)
#     Label(mntnce,text='Hey There! \n This window is Under Construction :(',fg='#0E185F',bg=w, font=('Microsoft YaHei Light',40) ).place(relx=.3,rely=.5)
#     Label(mntnce,text='But be assure, we will build it faster than EIP portal...',fg='#0E185F',bg=w, font=('Microsoft YaHei Light',15) ).place(relx=.3,rely=.7)
#     Label(mntnce,text='Till then, ',fg='#0E185F',bg=w, font=('Microsoft YaHei Light',15) ).place(relx=.3,rely=.74)
    
#     g=Label(mntnce,image=testsad,bg='white')
#     g.image=testsad
#     g.place(relx=.7,rely=.7)
    
#     def newtab1():
#         root.lift()
#         mntnce.destroy()

#     Button(mntnce,text='Bye..', borderwidth=0,bg=w,fg='#0E185F', command=newtab1, font=('Microsoft YaHei Light',15)).place(relx=.34,rely=.735)
    
     
# def newtab1(event = None):                    #function to back in the root window_____________________________________________                        
#     root.lift()                

# def popupm(g,p):                                #submenus of Instruments parameters_______________________________________________
                   
#     if  os.path.isfile('Additional_file\\temp\\Main_sheet.docx')== TRUE:                                                                                     
...
#             p.grab_release()
#     else:
#         messagebox.showerror('Action Prohibited',"Fill Main Sheet and Cover Sheet First!")
